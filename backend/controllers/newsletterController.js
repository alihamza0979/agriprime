...
exports.subscribe = async (req, res) => {
    try {
        ...
        try {
            ...
            `;
            await sendEmail(email, 'Welcome to AgriPrime Newsletter!', html);
        } catch (emailErr) {
            console.error('Failed to send newsletter welcome email:', emailErr);
            // We can still return success even if email fails due to bad config
        }
